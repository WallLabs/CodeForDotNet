...
namespace CodeChief.Net
{
    ...
    public static class MimeExtensions
    {
        ...
        /// <summary>
        /// Content Disposition header value for an inline file (open within browser if supported, e.g. Adobe PDF viewed in browser via ActiveX).
        ...
        public static bool FindLine(byte[] buffer, int offset, int length, out int lineSize, out int nextLine)
        {
            ...
            // Get line size, ingnoring any CR before LF
        ...
        public static bool FindBoundary(byte[] buffer, int offset, int length, byte[] boundary, out int contentEnd, out int boundaryEnd)
        {
            ...
            while (search < offset + length - boundary.Length + 2)
            {
                ...
                int lineSize;
                int nextLine;
                if (!FindLine(buffer, search, offset + length - search, out lineSize, out nextLine))
                    return false;
