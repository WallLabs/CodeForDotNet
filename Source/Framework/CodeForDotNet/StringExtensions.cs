...
public static class StringExtensions
{
    ...
    public static string FilterSpecial(this string value, char? placeholder = '?')
    {
        ...
        {
            var valid = rawChar is >= AsciiMinimumPrintable and <= AsciiMaximumPrintable;
            if (valid)
                _ = result.Append(rawChar);
            else if (placeholder.HasValue)
                _ = result.Append(placeholder.Value);
        }
        return result.ToString();
    }

    /// <summary>
    /// Searches a string array for the first index starting with the specified value, using a <see cref="StringComparison.OrdinalIgnoreCase"/> comparison mode.
    /// </summary>
    ...
    public static string[] SplitEscaped(this string text, char delimiter, char escape, bool removeEscape, bool removeEmptyEntries)
    {
        ...
        foreach (var character in text)
        {
            ...
            if (escaped)
            {
                ...
                if (!removeEscape)
                    _ = item.Append(escape);

                // Add the escaped character
                _ = item.Append(character);
    ...
    public static string TrimLines(this string value)
    {
        ...
        foreach (var line in value.Trim().Split('\n'))
            _ = result.AppendLine(line.Trim().Trim('\r'));
    ...
    /// <summary>
    /// Terminates a string with an ending (e.g. <see cref="Truncatellipsis"/> "...") if it exceeds a specific length. Useful for building summary list items
    ...
    public static string Truncate(this string value, int length, string ending)
    {
        ...
        _ = result.Append(value[..(useEnding ? length - endingLength : length)]);

        // Add ending (if required)
        if (useEnding)
            _ = result.Append(ending);
