...
public abstract class DataObject : PropertyStore, IDataObject
{
    ...
    /// <summary>
    /// List of all property names which changed during <see cref="IEventCache.SuspendEvents"/> since the last <see cref="Create"/> or <see cref="Update"/>.
    /// </summary>
    ...
    /// <summary>
    /// Caches the last <see cref="DataChanged"/> event action when events are suspended. Only the most significant action is cached.
    /// </summary>
    ...
    /// </summary>
    /// <remarks>
    /// Only valid when the <see cref="DataState"/> is <see cref="DataObjectState.None"/>. Sets the <see cref="DataState"/> to
    ...
    /// </summary>
    /// <remarks>
    /// Only valid when the current <see cref="DataState"/> is <see cref="DataObjectState.Current"/> or <see cref="DataObjectState.Changed"/>. Sets the
    ...
    public virtual void Delete()
    {
        lock (SyncRoot)
        {
            ...
            {
                // Validate state
                if (DataState is not DataObjectState.Current and
                not DataObjectState.Changed)
    ...
    /// </summary>
    /// <remarks>
    /// Only valid when the <see cref="DataState"/> is not <see cref="DataObjectState.Deleted"/>. Sets the <see cref="DataState"/> to
    /// <see cref="DataObjectState.Current"/> once successful.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// Only valid when the current <see cref="DataState"/> is <see cref="DataObjectState.Changed"/>. Sets the <see cref="DataState"/> to
    /// <see cref="DataObjectState.Current"/> once successful.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>Sets <see cref="DataState"/> property to <see cref="DataObjectState.Changed"/> when properties change.</remarks>
    protected void DoDataChanged(DataObjectChangeAction action, Guid? propertyId)
    {
        lock (SyncRoot)
        {
            ...
            if (EventsAreEnabled)
            {
                ...
                OnDataChanged(new DataObjectChangeEventArgs(action, propertyId.HasValue ? [propertyId.Value] : null!));
            }
            else
            {
                // Cache event...

                // Add any changed property to instance cache
    ...
    /// <summary>
    /// Handles change notification events, when this object is notified that a related data object has changed via the <see cref="NotifyChange"/> method.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    /// Called when events are resumed the last time, i.e. is not fired when nested. Fires the <see cref="DataChanged"/> event when changes are pending.
    /// </summary>
    protected override void OnEventsResumed()
    {
        lock (SyncRoot)
        {
            ...
            if (_lastChangeAction.HasValue)
            {
                ...
                var args = new DataObjectChangeEventArgs(_lastChangeAction.Value, [.. properties]);
