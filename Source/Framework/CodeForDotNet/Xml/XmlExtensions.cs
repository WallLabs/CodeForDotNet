...
public static class XmlExtensions
{
    ...
    public static bool IsValidInXml(this char value)
    {
        return
             ...
             value is (char)0x9 or    // Horizontal tab (ASCII 9 or '\t').
         (char)0xA or    // New line (ASCII 10 or '\n').
         (char)0xD or    // Carriage (ASCII 13 or '\r').

        // Check valid alphabet, whitespace, accents and signs...
        (>= (char)0x20 and <= (char)0xD7FF) or
        (>= (char)0xE000 and <= (char)0xFFFD);
    }

    /// <summary>
    /// Overload of the <see cref="XmlReader.ReadElementContentAsObject()"/> which behaves similarly to the XmlReader.ReadElementString() method. Required
    /// because the <see cref="XmlReader.ReadElementContentAsObject()"/> is not robust enough because it does not move to the next element before reading, so
    ...
    /// <summary>
    /// Overload of the <see cref="XmlReader.ReadElementContentAs(Type, IXmlNamespaceResolver)"/> which behaves similarly to the
    /// XmlReader.ReadElementString() method. Required because the <see cref="XmlReader.ReadElementContentAs(Type, IXmlNamespaceResolver)"/> is not robust
    ...
    /// <summary>
    /// Overload of the <see cref="XmlReader.ReadElementContentAsBase64(byte[], int, int)"/> which behaves similarly to the XmlReader.ReadElementString()
    /// method. Required because the <see cref="XmlReader.ReadElementContentAsBase64(byte[], int, int)"/> is not robust enough because it does not move to
    ...
    /// <summary>
    /// Overload of the <see cref="XmlReader.ReadElementContentAsBinHex(byte[], int, int)"/> which behaves similarly to the XmlReader.ReadElementString()
    /// method. Required because the <see cref="XmlReader.ReadElementContentAsBinHex(byte[], int, int)"/> is not robust enough because it does not move to
    /// the next element before reading, so fails if any other content (whitespace, comments, etc...) appears at the current position, just before the next
    /// element to read.
    /// </summary>
    /// <param name="reader">Extension target.</param>
    ...
    /// <summary>
    /// Overload of the <see cref="XmlReader.ReadElementContentAsBoolean()"/> which behaves similarly to the XmlReader.ReadElementString() method. Required
    /// because the <see cref="XmlReader.ReadElementContentAsBoolean()"/> is not robust enough because it does not move to the next element before reading,
    ...
    /// <summary>
    /// Overload of the <see cref="XmlReader.ReadElementContentAsDecimal()"/> which behaves similarly to the XmlReader.ReadElementString() method. Required
    /// because the <see cref="XmlReader.ReadElementContentAsDecimal()"/> is not robust enough because it does not move to the next element before reading,
    /// so fails if any other content (whitespace, comments, etc...) appears at the current position, just before the next element to read.
    /// </summary>
    /// <param name="reader">Extension target.</param>
    /// <param name="localName">
    ...
    /// <summary>
    /// Overload of the <see cref="XmlReader.ReadElementContentAsDouble()"/> which behaves similarly to the XmlReader.ReadElementString() method. Required
    /// because the <see cref="XmlReader.ReadElementContentAsDouble()"/> is not robust enough because it does not move to the next element before reading, so
    /// fails if any other content (whitespace, comments, etc...) appears at the current position, just before the next element to read.
    /// </summary>
    /// <param name="reader">Extension target.</param>
    /// <param name="localName">
    ...
    /// <summary>
    /// Overload of the <see cref="XmlReader.ReadElementContentAsInt()"/> which behaves similarly to the XmlReader.ReadElementString() method. Required
    /// because the <see cref="XmlReader.ReadElementContentAsInt()"/> is not robust enough because it does not move to the next element before reading, so
    /// fails if any other content (whitespace, comments, etc...) appears at the current position, just before the next element to read.
    /// </summary>
    /// <param name="reader">Extension target.</param>
    /// <param name="localName">
    ...
    /// <summary>
    /// Overload of the <see cref="XmlReader.ReadElementContentAsLong()"/> which behaves similarly to the XmlReader.ReadElementString() method. Required
    /// because the <see cref="XmlReader.ReadElementContentAsLong()"/> is not robust enough because it does not move to the next element before reading, so
    /// fails if any other content (whitespace, comments, etc...) appears at the current position, just before the next element to read.
    /// </summary>
    /// <param name="reader">Extension target.</param>
    /// <param name="localName">
    ...
    /// <summary>
    /// Overload of the <see cref="XmlReader.ReadElementContentAsFloat()"/> which behaves similarly to the XmlReader.ReadElementString() method. Required
    /// because the <see cref="XmlReader.ReadElementContentAsFloat()"/> is not robust enough because it does not move to the next element before reading, so
    /// fails if any other content (whitespace, comments, etc...) appears at the current position, just before the next element to read.
    /// </summary>
    /// <param name="reader">Extension target.</param>
    /// <param name="localName">
    ...
    public static string StripInvalidXmlChars(this string value)
    {
        ...
        return new string([.. value.ToCharArray().Where(IsValidInXml)]);
    }

    /// <summary>
    /// Overload of the <see cref="XmlWriter.WriteValue(object)"/> which behaves similarly to the <see cref="XmlWriter.WriteElementString(string, string)"/>
    ...
    /// <summary>
    /// Overload of the <see cref="XmlWriter.WriteValue(object)"/> which behaves similarly to the
    /// <see cref="XmlWriter.WriteElementString(string, string, string)"/> method. Helps reduce the amount of code necessary to write element values other
    /// than string types.
    /// </summary>
    /// <param name="writer">Extension target.</param>
    /// <param name="localName">Element name to write.</param>
    ...
    public static void WriteElementValue(this XmlWriter writer, string localName, string? @namespace, object value)
    {
        ...
        // Write element start (moving past any other non-element content) using the appropriate overload depending on parameters passed
        if (@namespace == null)
