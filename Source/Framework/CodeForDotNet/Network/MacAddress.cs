...
namespace CodeForDotNet.Network;
...
/// </summary>
public partial struct MacAddress : IFormattable, IComparable, IComparable<MacAddress>, IEquatable<MacAddress>
{
    /// <summary>
    ...
    /// <summary>
    /// Compiled regular expression for <see cref="StringValueExpression"/>.
    /// </summary>
    [GeneratedRegex(StringValueExpression)]
    ...
    /// <summary>
    /// Compiled regular expression for <see cref="StringFormatExpression"/>.
    /// </summary>
    [GeneratedRegex(StringFormatExpression)]
    public static partial Regex StringFormatRegEx();
    ...
    /// </summary>
    [XmlText]
    public readonly string ValueString => ToString();
    ...
    public readonly string ToString(string? format, IFormatProvider? formatProvider)
    {
        ...
        // Validate format
        if (!StringFormatRegEx().IsMatch(format))
            throw new ArgumentOutOfRangeException(nameof(format));

        // Get format style
        var style = format.ToUpperInvariant()[0];
        var reverse = char.IsLower(style);
        var group = format.Length == 2 ? int.Parse(format[1].ToString(CultureInfo.InvariantCulture), CultureInfo.InvariantCulture) : 0;
    ...
    {
        return other is MacAddress address
            ? CompareTo(address.ToInt64())
            : other is long longValue
            ? ToInt64().CompareTo(longValue)
            : other is byte[] bytes ? CompareTo(new MacAddress(bytes)) : throw new NotSupportedException();
    }

    /// <summary>
    /// Compares the current object with another object of the same type.
    /// </summary>
    public readonly int CompareTo(MacAddress other)
    {
    ...
    public override readonly bool Equals(object? other)
    {
        return other is MacAddress address
            ? Equals(address)
            : other is long longValue ? ToInt64().Equals(longValue) : other is byte[] bytes && Equals(new MacAddress(bytes));
    }

    /// <summary>
    /// Returns a value that indicates whether this instance is equal to a specified object.
    /// </summary>
    public readonly bool Equals(MacAddress other)
    {
    ...
    public readonly byte[] ToByteArray(bool reversed)
    {
    ...
    private static long? TryParseInt64(string mac, bool throwExceptions)
    {
        ...
        if (string.IsNullOrEmpty(mac))
            return throwExceptions ? throw new ArgumentNullException(nameof(mac)) : null;

        // Check format
        if (!StringValueRegEx().IsMatch(mac))
            return throwExceptions ? throw new FormatException() : null;
        ...
        if (!long.TryParse(mac, NumberStyles.HexNumber, CultureInfo.InvariantCulture, out var result))
            return throwExceptions ? throw new FormatException() : null;
