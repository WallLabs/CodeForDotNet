...
public abstract class ApplicationBase : Application
{
    ...
    /// </summary>
    protected static void OnError(object sender, Windows.UI.Xaml.UnhandledExceptionEventArgs error)
    {
        // Validate.
        ArgumentNullException.ThrowIfNull(error);

        // Store error to send later
        _ = LocalErrorStore.Add(error.Exception);
    }
    ...
    protected override void OnLaunched(LaunchActivatedEventArgs launch)
    {
        ...
        if (Window.Current.Content is not Frame rootFrame)
        {
            ...
            if (launch.PreviousExecutionState == ApplicationExecutionState.Terminated)
            {
        ...
        {
            if (!rootFrame.Navigate(StartPageType, launch.Arguments))
    ...
    /// <param name="navgation">Details about the navigation failure</param>
    private void OnNavigationFailed(object sender, NavigationFailedEventArgs navgation)
    {
        throw new InvalidOperationException("Failed to load Page " + navgation.SourcePageType.FullName);
    }

    /// <summary>
    /// Saves state when the application is suspended.
    /// </summary>
    protected void OnSuspending(object sender, SuspendingEventArgs suspend)
    {
        // Validate.
        ArgumentNullException.ThrowIfNull(suspend);

        // Delay
        var deferral = suspend.SuspendingOperation.GetDeferral();
        try
        {
            // Save state
            SaveState();
    ...
    /// </summary>
    protected void OnResuming(object? sender, object resume)
    {
        try
        {
            // Load state
