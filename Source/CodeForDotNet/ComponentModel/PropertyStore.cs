...
public abstract class PropertyStore : EventCache, IPropertyStore
{
    ...
    public T GetProperty<T>(Guid id)
    {
        lock (SyncRoot)
        {
            return _properties.TryGetValue(id, out var value) ? (T)value : throw new ArgumentOutOfRangeException(nameof(id));
        }
    }
    ...
    public T GetProperty<T>(Guid id, T defaultValue)
    {
        lock (SyncRoot)
        {
            return _properties.TryGetValue(id, out var value) ? (T)value : defaultValue;
        }
    }
    ...
    public Collection<Guid> GetPropertyIds()
    {
        lock (SyncRoot)
        {
            return new Collection<Guid>([.. _properties.Keys]);
        }
    }
    ...
    protected override void OnEventsResumed()
    {
        lock (SyncRoot)
        {
            ...
            {
                // Build event
                var args = new PropertyStoreChangeEventArgs([.. _changedProperties]);
    ...
    private void DoPropertyChanged(Guid propertyId)
    {
        lock (SyncRoot)
        {
            ...
            {
                // Fire event
                OnPropertyChanged(new PropertyStoreChangeEventArgs([propertyId]));
            }
            else
            {
                // Cache event
