...
{
   /// <summary>
   /// Base object which provides a compact <see cref="INotifyPropertyChanged"/> implementation. Use
   /// the
   /// <see cref="SetProperty&lt;T&gt;(ref T, T, string, SetPropertyEventHandler&lt;T&gt;, SetPropertyEventHandler&lt;T&gt;)"/>
   /// method in your property setters to automatically test for equality, then set the new value and
   /// fire the <see cref="PropertyChanged"/> event when changed.
   /// </summary>
   public abstract class NotifyPropertyChangedBase : EventCache, INotifyPropertyChanged
   {
      ...
      /// <summary>
      /// Synchronization context under which this object was created, e.g. parent UI dispatcher such
      /// as a DependencyObject context in XAML.
      /// </summary>
      ...
      /// Property name, must match public property name else bindings will not see change.
      /// </param>
      ...
      /// Optional callback which is invoked before assignment and before firing the changed event,
      /// with the old and new values, e.g. to un-hook events or deactivate the old value.
      /// </param>
      /// <param name="afterAssign">
      /// Optional callback which is invoked after assignment but before firing the changed event,
      /// with the old and new values, e.g. to hook events or clean-up the old value.
      /// </param>
      /// <returns>True when changed.</returns>
      protected bool SetProperty<T>(ref T target, T value, string propertyName,
