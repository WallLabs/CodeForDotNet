...
public static class MimeExtensions
{
    ...
    public static bool FindLine(byte[] buffer, int offset, int length, out int lineSize, out int nextLine)
    {
        ...
        lineSize = newLineIndex > 0 && buffer[newLineIndex - 1] == (byte)'\r' ? newLineIndex - offset - 1 : newLineIndex - offset;
    ...
    public static string? GetHeaderAttribute(string header, string name)
    {
        ...
        var attributes = header.Split([';'], StringSplitOptions.RemoveEmptyEntries);
    ...
    public static string? GetHeaderValue(string header)
    {
        ...
        // Strip header value
        var firstAttribute = header.IndexOf(';', StringComparison.OrdinalIgnoreCase);
        if (firstAttribute > 0)
            header = header[..firstAttribute];
        return header.Trim();
    }
