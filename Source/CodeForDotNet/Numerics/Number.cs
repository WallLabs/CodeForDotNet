...
public struct Number : IComparable<Number>, IEquatable<Number>
{
    ...
    /// </summary>
    public static Number MinusOne => new([0xff], true);

    /// <summary>
    /// One signed value.
    /// </summary>
    public static Number One => new([0x01], true);

    /// <summary>
    /// Zero signed value.
    /// </summary>
    public static Number Zero => new([], true);

    /// <summary>
    /// Size in bytes.
    /// </summary>
    public readonly int ByteSize => _bytes != null ? _bytes.Length : 0;
    ...
    public static explicit operator byte(Number value)
    {
        ...
        return byteLength > 1 || value.Signed ? throw new OverflowException() : byteLength == 0 ? (byte)0 : value._bytes[0];
    }
    ...
    {
        var byteLength = value.ByteSize;
        return byteLength > 1 ? throw new OverflowException() : byteLength == 0 ? (sbyte)0 : (sbyte)value._bytes[0];
    }
    ...
    public static Number RightShift(Number value, int shift)
    {
        unchecked
        {
            ...
            if (shiftWholeBytes >= valueSize)               // Return zero when entire value shifted out
                return new Number([], value.Signed);
    ...
    /// </remarks>
    public readonly ReadOnlyCollection<byte> GetBytes()
    {
    ...
    public readonly Number Resize(int size)
    {
    ...
    public readonly Number Resize(int size, bool extendSign)
    {
        // Resize
    ...
    public readonly byte ToByte()
    {
        ...
        return byteLength > 1 || Signed ? throw new OverflowException() : byteLength == 0 ? (byte)0 : _bytes[0];
    }
    ...
    {
        var byteLength = ByteSize;
        return byteLength > 1 ? throw new OverflowException() : byteLength == 0 ? (sbyte)0 : (sbyte)_bytes[0];
    }
    ...
    public readonly Number ToSigned(bool extend)
    {
    ...
    /// Converts the number to a string of the specified base.
    /// </summary>
    public readonly string ToString(int numberBase, int minimumLength)
    {
        unchecked
        {
            // Validate
            ...
            for (var byteIndex = 0; byteIndex < byteLength; byteIndex++)
            {
                ...
                do
                {
                    ...
                    _ = result.Insert(firstDigitIndex, [NumberDigits[digitValue]]);
                    digits++;
                    digitsThisByte++;
            ...
            while (result.Length - firstDigitIndex < minimumLength)
                _ = result.Insert(firstDigitIndex, [negative && !negate ? NumberDigits[numberBase - 1] : NumberDigits[0]]);
    ...
    /// </returns>
    public readonly Number ToUnsigned(bool positiveOnly)
    {
