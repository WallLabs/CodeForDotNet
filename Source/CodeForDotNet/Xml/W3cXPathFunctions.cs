...
namespace CodeForDotNet.Xml
{
	...
	public sealed class W3cXPathFunctions
	{
		...
		/// <param name="pattern">Regular expression pattern to match.</param>
		/// <param name="flags">
		/// Regular expression flag characters, one or more of: s - Single-line mode. Corresponds to <see cref="RegexOptions.Singleline"/>. m - Multi-line mode.
		/// Corresponds to <see cref="RegexOptions.Multiline"/>. i - Case insensitive match. Corresponds to <see cref="RegexOptions.IgnoreCase"/>. x - Ignore
		/// whitespace in pattern, e.g. when spaces or new lines are used in long patterns to make it readable. Corresponds to <see cref="RegexOptions.IgnorePatternWhitespace"/>.
		/// </param>
		/// <returns>True when matched.</returns>
		/// <remarks>
		/// The <paramref name="flags"/> parameter cannot have a default to replace the <see cref="matches(string, string)"/> method because the XSLT extensions
