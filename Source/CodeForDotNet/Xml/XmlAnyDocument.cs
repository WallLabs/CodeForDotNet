...
namespace CodeForDotNet.Xml
{
   ...
   /// </summary>
   /// <remarks>
   /// The default root element is "root" when serialized standalone. Normally this is a property of
   /// another serializable type, in which case the root is set by the XML serialization attributes
   /// of the parent property or in the custom serialization code of the parent class if implemented.
   /// </remarks>
   [Serializable]
   ...
   public sealed class XmlAnyDocument : ISerializable, IXmlSerializable, IDisposable
   {
      ...
      /// </summary>
      /// <remarks>
      /// Even if this instance was serialized standalone the default "root" element is not part of
      /// the path because it is stripped during de-serialization.
      /// </remarks>
      /// <param name="path">
      ...
      /// <summary>
      /// Returns an <see cref="XPathNavigator"/> positioned within the data. Parent elements in the
      /// path without filters will be created if the <paramref name="create"/> option is set. (up to
      /// the first filter).
      /// </summary>
      /// <remarks>
      /// Even if this instance was serialized standalone the default "root" element is not part of
         ...
         if (create && result == null)
         {
            ...
            foreach (var pathPart in path.Split(new[] { '/' }, StringSplitOptions.RemoveEmptyEntries))
            {
               ...
               if (result != null)
               {
                  ...
                  if (child == null)
                  {
                     // Return null when path includes filter (cannot automatically create
                     // conditional content)
      ...
      /// True when called proactively by <see cref="Dispose()"/>. False when called during finalization.
      /// </param>
