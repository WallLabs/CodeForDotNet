...
namespace CodeForDotNet.Drawing
{
    ...
    public class LogicalBrush : ICloneable
    {
        ...
        /// </summary>
        public LogicalBrush(byte[] texture, Decimal angle, WrapMode wrapMode, float scale)
        {
        ...
        /// </summary>
        public Brush MakeBrush(Rectangle bounds)
        {
            return MakeBrush(bounds, false);
        }
        ...
        /// </summary>
        public Brush MakeBrush(Rectangle bounds, bool reverse)
        {
            ...
            switch (_brushType)
            {
                case LogicalBrushType.SingleColor:
                    {
                        // Single Color
                        if (!StartColor.HasValue) throw new ArgumentNullException(
                            String.Format(CultureInfo.CurrentCulture, Resources.PropertyRequired, "StartColor"));
                        return new SolidBrush(Color.FromArgb(StartColor.Value));
                    }
                ...
                case LogicalBrushType.TwoColorGradient:
                    {
                        ...
                        if (!StartColor.HasValue) throw new ArgumentNullException(
                            String.Format(CultureInfo.CurrentCulture, Resources.PropertyRequired, "StartColor"));
                        if (!EndColor.HasValue) throw new ArgumentNullException(
                            String.Format(CultureInfo.CurrentCulture, Resources.PropertyRequired, "EndColor"));
                        if (!Angle.HasValue) throw new ArgumentNullException(
                            String.Format(CultureInfo.CurrentCulture, Resources.PropertyRequired, "Angle"));
                        Color startColor, endColor;
                        if (reverse)
                        {
                            startColor = Color.FromArgb(EndColor.Value);
                            endColor = Color.FromArgb(StartColor.Value);
                        }
                        else
                        {
                            startColor = Color.FromArgb(StartColor.Value);
                            endColor = Color.FromArgb(EndColor.Value);
                        }
                        return new LinearGradientBrush(bounds, startColor,
                                                         endColor, Decimal.ToSingle(Angle.Value));
                    }
                ...
                case LogicalBrushType.Texture:
                    {
                        ...
                        if (!Angle.HasValue) throw new ArgumentNullException(
                            String.Format(CultureInfo.CurrentCulture, Resources.PropertyRequired, "Angle"));
                        if (!WrapMode.HasValue) throw new ArgumentNullException(
                            String.Format(CultureInfo.CurrentCulture, Resources.PropertyRequired, "WrapMode"));
                        if (!Scale.HasValue) throw new ArgumentNullException(
                            String.Format(CultureInfo.CurrentCulture, Resources.PropertyRequired, "Scale"));

                        // Make texture
                        Image image;
                        using (var reader = new MemoryStream(Texture))
                            image = Image.FromStream(reader);
                        var brush = new TextureBrush(image, WrapMode.Value);

                        // Rotate
                        brush.RotateTransform(Decimal.ToSingle(Angle.Value), MatrixOrder.Append);
                        ...
                        // Scale image
                        var fFinalScale = Scale.Value * fScale;
