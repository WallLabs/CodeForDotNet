...
namespace CodeForDotNet.Windows.Native
{
    ...
    public abstract class NativeCodeGenerator
    {
        ...
        static private string GetNgenPath()
        {
            ...
            var size = 0;
            var rc = SafeNativeMethods.GetCORSystemDirectory(path, path.Capacity, ref size);
            if (rc != 0)
        ...
        public static bool Install(string assemblyPath, out string consoleOutput, out string assemblyFullName)
        {
            ...
            var rc = ExecuteProgram(GetNgenPath(), parameters, Path.GetDirectoryName(assemblyPath), 5, out consoleOutput);
            if (rc == 0)
            {
        ...
        public static bool Remove(string assemblyName, out string consoleOutput)
        {
            ...
            var rc = ExecuteProgram(program, parameters, Path.GetDirectoryName(program), 5, out consoleOutput);
