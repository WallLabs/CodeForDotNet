...
public partial class DynamicTextBox : TextBox
{
    ...
    /// </summary>
    public void HandleKeyDown(KeyRoutedEventArgs arguments)
    {
        OnKeyDown(arguments);
    }
    ...
    public bool MoveDownInGroup()
    {
        ...
        // Focus new control
        _ = control.Focus(FocusState.Keyboard);
    ...
    /// </summary>
    protected virtual void OnAfterKeyDown(KeyRoutedEventArgs arguments)
    {
    }
    ...
    /// </summary>
    protected virtual void OnAfterKeyUp(KeyRoutedEventArgs arguments)
    {
    }
    ...
    /// </summary>
    protected virtual void OnAfterTextChanged(object sender, DynamicTextChangedEventArgs arguments)
    {
    }
    ...
    /// </summary>
    protected virtual void OnBeforeKeyDown(KeyRoutedEventArgs arguments)
    {
        // Validate.
    ...
    /// </summary>
    protected virtual void OnBeforeKeyUp(KeyRoutedEventArgs arguments)
    {
    }
    ...
    /// </summary>
    protected virtual void OnBeforeSelectionChanged(object sender, DynamicTextSelectionChangedEventArgs arguments)
    {
        // Validate.
        ArgumentNullException.ThrowIfNull(arguments);
        ...
        if (maxLength > 0 && textLength >= maxLength)
        {
            ...
            {
                arguments.SelectionStart = selectedIndex - 1;
            }

            // Select one character
            arguments.SelectionLength = 1;
        }
    ...
    /// </summary>
    protected virtual void OnBeforeTextChanged(object sender, DynamicTextChangedEventArgs arguments)
    {
        // Validate.
        ArgumentNullException.ThrowIfNull(arguments);
    ...
    protected override void OnKeyDown(KeyRoutedEventArgs arguments)
    {
        // Fire before event
        BeforeKeyDown?.Invoke(this, arguments);

        // Call before event handler
        OnBeforeKeyDown(arguments);
        ...
        if (!arguments.Handled)
        {
            if (DirectInput)
            {
                ...
                var inputText = KeyboardKeyConverter.ConvertToString((KeyboardKey)arguments.Key, upperCase, CultureInfo.CurrentCulture);
                if (inputText != null)
                {
        ...
        if (!_keysDown.Contains(arguments.Key))
        {
            _keysDown.Add(arguments.Key);
        }

        // Call after event handler
        OnAfterKeyDown(arguments);
    }
    ...
    /// </summary>
    [SuppressMessage("Naming", "CA1725:Parameter names should match base declaration", Justification = "Readability.")]
    protected override void OnKeyUp(KeyRoutedEventArgs arguments)
    {
        // Validate.
        ArgumentNullException.ThrowIfNull(arguments);

        // Call before event handler
        OnBeforeKeyUp(arguments);

        // Call base class method to process key release unless blocked
        if (!arguments.Handled)
        {
            base.OnKeyUp(arguments);
        }

        // Track pressed keys (remove from keys down)
        if (_keysDown.Contains(arguments.Key))
        {
            _ = _keysDown.Remove(arguments.Key);
        }

        // Call after event handler
        OnAfterKeyUp(arguments);
    }
    ...
    /// </summary>
    private void OnSelectionChanged(object sender, RoutedEventArgs arguments)
    {
        // Apply rules
        ...
        if (newFocus != this)
        {
            _ = newFocus.Focus(FocusState.Keyboard);
            return;
        }
    ...
    /// </summary>
    private void OnTextChanged(object sender, TextChangedEventArgs arguments)
    {
