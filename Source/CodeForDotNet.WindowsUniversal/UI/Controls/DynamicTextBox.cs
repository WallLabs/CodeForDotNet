...
{
    /// <summary>
    /// Enhanced <see cref="TextBox"/> control supporting multiple key-down tracking and group
    ...
    public class DynamicTextBox : TextBox
    {
        ...
        /// <summary>
        /// Enables direct handling of input from key press events, allowing input to be accepted
        /// even when the control has <see cref="TextBox.IsReadOnly"/> set, e.g. to blocking any soft
        /// keyboards from popping up.
        /// </summary>
        ...
        /// <summary>
        /// Sets the next control downwards in an input group, to which the focus will be switched
        /// when moving down in this text box.
        /// </summary>
        ...
        /// <summary>
        /// Sets the next control in an input group, to which the focus will be switched when moving
        /// right at the edge of this text box or typing a character beyond the maximum length.
        /// </summary>
        ...
        /// <summary>
        /// Sets the previous control in an input group, to which the focus will be switched when
        /// moving left or typing at the edge of this text box.
        /// </summary>
        ...
        /// <summary>
        /// Sets the previous control upwards an input group, to which the focus will be switched
        /// when moving up in this text box.
        /// </summary>
        ...
        /// <summary>
        /// Forces all alphabetical keys to upper case, not shift. Requires <see cref="DirectInput"/>
        /// to be enabled.
        /// </summary>
        ...
        /// <summary>
        /// Currently pressed keys. Set during the <see cref="OnKeyDown"/> even handler, after
        /// <see cref="OnBeforeKeyDown"/> but before <see cref="OnAfterKeyDown"/> to supporting
        /// repeat detection.
        /// </summary>
        ...
        public void DoBackspace()
        {
            ...
            string text = Text;
            int selectLength = SelectionLength;
            int selectIndex = SelectionStart;
        ...
        /// <summary>
        /// Moves to the <see cref="GroupDown"/> control if specified. When the next control is a
        /// <see cref="TextBox"/> also selects the next logical character from which to continue input.
        /// </summary>
        /// <returns>True when a move occurred.</returns>
        public bool MoveDownInGroup()
        {
            // Check if control specified, return negative when not
            Control control = GroupDown;
            if (control == null)
            {
                return false;
            }
        ...
        /// <summary>
        /// Moves to the <see cref="GroupPrevious"/> control if specified. When the next control is a
        /// <see cref="TextBox"/> also selects the next logical character from which to continue input.
        /// </summary>
        /// <returns>True when a move occurred.</returns>
        public bool MoveLeftInGroup()
        {
            ...
            if (control is TextBox textBox)
            {
                ...
                int textMax = textBox.MaxLength;
                int textLength = textBox.Text.Length;
                int selectStart = textLength > 0 && textMax > 0 && textLength >= textMax ? textLength - 1 : textLength;
                int selectLength = SelectionLength;
        ...
        /// <summary>
        /// Moves to the <see cref="GroupNext"/> control if specified. When the next control is a
        /// <see cref="TextBox"/> also selects the next logical character from which to continue input.
        /// </summary>
        /// <returns>True when a move occurred.</returns>
        ...
        /// <summary>
        /// Moves to the <see cref="GroupUp"/> control if specified. When the next control is a
        /// <see cref="TextBox"/> also selects the next logical character from which to continue input.
        /// </summary>
        /// <returns>True when a move occurred.</returns>
        ...
        /// <summary>
        /// Called after the <see cref="UIElement.KeyDown"/> event is processed. The key press was
        /// blocked when <see cref="KeyRoutedEventArgs.Handled"/> is true.
        /// </summary>
        ...
        /// <summary>
        /// Called after the <see cref="UIElement.KeyUp"/> event is processed. The key release
        /// processing was blocked when <see cref="KeyRoutedEventArgs.Handled"/> is true.
        /// </summary>
        ...
        protected override void OnApplyTemplate()
        {
            ...
            if (_cursor == null)
            {
                Grid grid = (Grid)VisualTreeHelper.GetChild(this, 0);
                Canvas canvas = new Canvas { Background = new SolidColorBrush(Color.FromArgb(0, 0, 0, 0)) };
        ...
        /// <summary>
        /// Called before the <see cref="UIElement.KeyDown"/> event is processed. Set
        /// <see cref="KeyRoutedEventArgs.Handled"/> true to block the key press from being processed
        /// by the base control. Use the <see cref="KeyRoutedEventArgs.Key"/> to examine the current
        /// key press, the <see cref="KeysDown"/> collection does not contain the current key unless
        /// it was previously pressed to support repeat detection. This class implements grouped
        /// control navigation in this method. If you override this method and do not call it this
        ...
        protected virtual void OnBeforeKeyDown(KeyRoutedEventArgs args)
        {
            ...
            bool overtype = MaxLength > 0 && textLength >= MaxLength;
            int selectionMax = overtype ? textLength - 1 : textLength;
            bool shift = KeysDown.Contains(VirtualKey.Shift) ||
        ...
        /// <summary>
        /// Called before the <see cref="UIElement.KeyUp"/> event is processed. Set
        /// <see cref="KeyRoutedEventArgs.Handled"/> true to block the key release from being
        /// processed by the base control. Use the <see cref="KeyRoutedEventArgs.Key"/> to examine
        /// the released key, the <see cref="KeysDown"/> collection is the state before processing
        /// this event so still contains the released key.
        /// </summary>
        ...
        protected virtual void OnBeforeSelectionChanged(object sender, DynamicTextSelectionChangedEventArgs args)
        {
            ...
            Control nextControl = GroupNext;
            int textLength = Text.Length;
            int selectedIndex = args.SelectionStart;
        ...
        /// <summary>
        /// Called before the base text box changed event is processed but this class, but after the
        /// core text box has already processed a change (because it provides no text change handler
        /// itself to override). This class performs trimming or padding here, so must be called when
        /// overridden to retain this functionality.
        /// </summary>
        ...
        protected override void OnKeyDown(KeyRoutedEventArgs args)
        {
            ...
            if (!args.Handled)
            {
                if (DirectInput)
                {
                    ...
                    string inputText = KeyboardKeyConverter.ConvertToString((KeyboardKey)args.Key, upperCase, CultureInfo.CurrentCulture);
                    if (inputText != null)
                    {
        ...
        private void LayoutCursor()
        {
            ...
            int cursorIndex = SelectionStart;
            bool trailing = false;
            ...
            Rect cursorRect = GetRectFromCharacterIndex(cursorIndex, trailing);
        ...
        {
            // Apply rules
            DynamicTextSelectionChangedEventArgs change = new DynamicTextSelectionChangedEventArgs(this, SelectionStart, SelectionLength);
            OnBeforeSelectionChanged(this, change);

            // Change focus when necessary
            Control newFocus = change.Focus;
            if (newFocus != this)
            {
        ...
        private void OnTextChanged(object sender, TextChangedEventArgs args)
        {
            ...
            int originalTextLength = _originalText.Length;
            int selectionOffset = originalTextLength - _originalSelectionStart;

            // Call before event handler
            DynamicTextChangedEventArgs change = new DynamicTextChangedEventArgs(Text, _originalText);
            ...
            // Restore selection
            int textLength = change.Text.Length;
            int selectionStart = textLength - selectionOffset;
            if (selectionStart < 0)
            {
                selectionStart = 0;
            }
